...
const textBundle = {
  ar: {
    ...
    about_goal_text: "تجميع فعاليات المدن السورية في مكان واحد ليسهل على الجمهور اكتشاف الأنشطة الثقافية والرياضية والعائلية.",
    about_vision_title: "الرؤية",
    about_vision_text: "أن يكون الدليل مرجعًا مبسّطًا وحديثًا يعكس حيوية المجتمع المحلي ويشجع المشاركة.",
    about_mission_title: "الرسالة",
    about_mission_text: "عرض معلومات دقيقة وبأسلوب واضح، مع تنظيم فعّال يراعي سهولة الوصول والتجاوب على مختلف الأجهزة.",
    team_title: "فريق العمل",
    team_name: "الاسم",
    team_class: "الصف",
    team_role: "الدور",
    policies_title: "سياسات مختصرة",
    policy_1: "تقديم فعالية يجب أن يتضمن: الاسم، التاريخ، المدينة، منظّم الحدث، صورة واضحة.",
  ...
  en: {
    ...
    hero_subtitle: "Music, culture, sports, and family shows — all in one place.",
    ...
    about_goal_text: "Aggregate events in Syrian cities in one place to make cultural, sports, and family activities easy to discover.",
    about_vision_title: "Vision",
    about_vision_text: "Be a simple, up-to-date reference that reflects community vitality and encourages participation.",
    about_mission_title: "Mission",
    about_mission_text: "Provide accurate information in a clear style with responsive, accessible organization.",
    team_title: "Team",
    team_name: "Name",
    team_class: "Class",
    team_role: "Role",
    policies_title: "Short policies",
    policy_1: "Submitting an event must include: name, date, city, organizer, and a clear image.",
    policy_2: "Concise and clear content is preferred, including available contact channels.",
    policy_3: "We review data before publishing to ensure consistency and formatting.",
...
const categoryDict = {
  ...
  culture: { ar: "ثقافي", en: "Culture" },
  music:  { ar: "موسيقى", en: "Music" },
  sports: { ar: "رياضة", en: "Sports" },
};

const getCatLabel = (key) => categoryDict[key]?.[runtimeState.lang] ?? key;
...
function applyLanguage(lang) {
  ...
  const brandEl = document.querySelector(".navbar-brand[data-i18n='brand']");
  if (brandEl) brandEl.textContent = textBundle[lang].brand;
...
function setupThemeButtons() {
  ...
  if (btnL && btnD) {
    btnL.addEventListener("click", () => { runtimeState.theme = "light"; setCookie("theme", "light"); setActive(); });
    btnD.addEventListener("click", () => { runtimeState.theme = "dark";  setCookie("theme", "dark");  setActive(); });
    setActive();
  }
}
...
function asDate(str) { return new Date(str + "T00:00:00"); }
function isThisWeek(date) {
  const now = new Date();
  const in7 = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 7);
  return date >= now && date <= in7;
}
const cityName = (ev) => runtimeState.lang === "ar" ? (ev.city_ar || ev.city || "") : (ev.city_en || ev.city || "");
const venueName = (ev) => {
  const vAr = ev.venue_ar || ev.venue;
  const vEn = ev.venue_en || ev.venue;
  return runtimeState.lang === "ar" ? vAr : vEn;
};
const titleOf = (ev) => runtimeState.lang === "ar" ? ev.title_ar : ev.title_en;
const summaryOf = (ev) => runtimeState.lang === "ar" ? ev.summary_ar : ev.summary_en;
const descOf = (ev) => runtimeState.lang === "ar" ? ev.desc_ar : ev.desc_en;
...
async function setupHomePage() {
  ...
  if (catsRow) {
    catsRow.innerHTML = "";
    uniqueCategories(data).forEach((key) => {
      const href = "events.html#cat=" + encodeURIComponent(key);
      catsRow.insertAdjacentHTML(
        "beforeend",
        `<div class="col-6 col-md-3 col-lg-2">
           <a href="${href}" class="btn btn-outline-primary w-100">${getCatLabel(key)}</a>
         </div>`
      );
    });
  }
  ...
  if (latestRow) {
    latestRow.innerHTML = "";
    const latest = [...data].sort((a, b) => asDate(b.date) - asDate(a.date)).slice(0, 3);
    latest.forEach((ev) => {
      ...
      latestRow.insertAdjacentHTML(
        ...
        `<div class="col-12 col-md-4">
           <div class="card h-100 text-center">
             <div class="pt-3"><img class="thumb-circle-lg" src="${ev.thumb}" alt="${title}"></div>
             <div class="card-body">
               <span class="card-title-ribbon mb-2">${title}</span>
               <p class="small text-body-secondary mb-1">${cityName(ev)} — ${ev.date}</p>
               <p class="mb-0">${summary}</p>
             </div>
             <div class="card-footer bg-transparent border-0 pb-3">
               <a href="event.html?id=${encodeURIComponent(ev.id)}" class="btn btn-sm btn-primary">${textBundle[runtimeState.lang].details}</a>
             </div>
           </div>
         </div>`
      );
    ...
    list.forEach((ev, idx) => {
      ...
      inner.insertAdjacentHTML(
        ...
        `<div class="carousel-item ${active}">
           <img src="${(ev.gallery?.[0] || ev.thumb)}" class="d-block w-100" alt="${title}" style="max-height:420px;object-fit:cover;">
           <div class="carousel-caption d-none d-md-block bg-body-tertiary bg-opacity-75 rounded-3 p-2">
             <h5 class="mb-1">${title}</h5>
             <p class="mb-1">${summary}</p>
             <p class="small mb-2">${cityName(ev)} — ${ev.date}</p>
             <a class="btn btn-sm btn-primary" href="event.html?id=${encodeURIComponent(ev.id)}">${textBundle[runtimeState.lang].details}</a>
           </div>
         </div>`
      );
    });
...
async function setupEventsPage() {
  ...
  if (sel) {
    sel.innerHTML = `<option value="" selected>${textBundle[runtimeState.lang].all}</option>`;
    uniqueCategories(data).forEach((key) => {
      const opt = document.createElement("option");
      opt.value = key;
      opt.textContent = getCatLabel(key);
      sel.appendChild(opt);
    });
  }


  if (location.hash.startsWith("#cat=") && sel) {
    sel.value = decodeURIComponent(location.hash.split("=")[1] || "");
  }

  const form = document.getElementById("filtersForm");
  if (form) form.addEventListener("submit", (e) => { e.preventDefault(); drawEventsList(data); });
...
function drawEventsList(data) {
  ...
  const filtered = data.filter((ev) => {
    ...
    const title = titleOf(ev).toLowerCase();
    const inTxt = q ? title.includes(q) || cityName(ev).toLowerCase().includes(q) : true;
    return inCat && inDate && inTxt;
  });

  list.innerHTML = "";
  if (!filtered.length) {
    list.innerHTML = `<div class="col-12"><div class="alert alert-warning">${textBundle[runtimeState.lang].no_results}</div></div>`;
    return;
  }

  filtered.forEach((ev) => {
    const title = titleOf(ev);
    const catBadge = getCatLabel(ev.category);
    list.insertAdjacentHTML(
      "beforeend",
      `<div class="col-12 col-md-6 col-lg-4">
         <div class="card h-100 text-center">
           <div class="pt-3"><img src="${ev.thumb}" class="thumb-circle-md" alt="${title}"></div>
           <div class="card-body">
             <div><span class="card-title-ribbon">${title}</span></div>
             <p class="small text-body-secondary mb-2">${cityName(ev)} — ${ev.date}</p>
             <div class="mb-2"><span class="badge text-bg-primary">${catBadge}</span></div>
             <a href="event.html?id=${encodeURIComponent(ev.id)}" class="btn btn-sm btn-primary">${textBundle[runtimeState.lang].details}</a>
           </div>
         </div>
       </div>`
...
async function setupEventPage() {
  ...
  if (!ev) {
    container.innerHTML = `<div class="alert alert-danger">${runtimeState.lang === "ar" ? "الفعالية غير موجودة" : "Event not found"}</div>`;
    return;
  }

  const title = titleOf(ev);
  const desc = descOf(ev);
  const catBadge = getCatLabel(ev.category);

  container.innerHTML = `
    <div class="row g-4">
      <div class="col-12">
        <img class="event-hero" src="${(ev.gallery?.[0] || ev.thumb)}" alt="${title}">
      </div>

      <div class="col-lg-8">
        <h1 class="h4 mb-2">${title}</h1>
        <p class="text-body-secondary small mb-3">${cityName(ev)} — ${venueName(ev)} — ${ev.date}</p>
        <p>${desc}</p>

        <h2 class="h6 mt-4">${textBundle[runtimeState.lang].gallery}</h2>
        <div class="d-flex flex-wrap gap-2">
          ${(ev.gallery || []).slice(0, 4).map((src) => `<img src="${src}" alt="${title}" class="rounded" style="width:220px;height:130px;object-fit:cover;">`).join("")}
        </div>

        <div class="mt-4 d-flex flex-wrap gap-2">
          <span class="badge text-bg-primary">${catBadge}</span>
          <span class="badge text-bg-secondary">${ev.organizer}</span>
        </div>

        <div class="mt-4 d-flex flex-wrap gap-2">
          <button class="btn btn-outline-secondary btn-sm" id="btnAddCalendar"><i class="bi bi-calendar-plus"></i> ${runtimeState.lang === "ar" ? "إضافة إلى التقويم" : "Add to calendar"}</button>
          <button class="btn btn-outline-primary btn-sm" id="btnShare"><i class="bi bi-share"></i> ${runtimeState.lang === "ar" ? "مشاركة" : "Share"}</button>
        </div>
        <div class="mt-2 alert alert-success d-none" id="shareAlert">${textBundle[runtimeState.lang].copied}</div>

        <div class="mt-4">
          <a href="events.html" class="btn btn-outline-secondary">${textBundle[runtimeState.lang].back_to_events}</a>
        </div>
      </div>
      ...
      <div class="col-lg-4">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">${textBundle[runtimeState.lang].approx_location}</h5>
...
function setupContactPage() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const valid = name.length >= 2 && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) && msg.length >= 5;
